...
class TestImageProcessor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Mock the YOLO model to avoid real inference and downloads
        self.processor.yolo_model = MagicMock()
        self.processor.yolo_model.names = {
            0: "person", 1: "dog", 2: "cat", 3: "car", 4: "stop sign"
        }
...
class TestAnnotateDetections(TestImageProcessor):
    ...
    @patch("cv2.imwrite")
    @patch("os.makedirs")
    def test_annotate_detections_saves_output(
        self, _mock_makedirs, _mock_imwrite
    ):
    ...
    @patch("cv2.imwrite")
    @patch("os.makedirs")
    def test_annotate_detections_colors(
        self, _mock_makedirs, _mock_imwrite, mock_rectangle, mock_load_image
    ):
        ...
        res = self.processor.annotate_detections("dummy.jpg", "out.jpg")

        self.assertTrue(res["saved"])
        self.assertEqual(res["person_count"], 1)
        self.assertEqual(res["animal_count"], 1)
        self.assertEqual(res["other_count"], 1)

        # Verify cv2.rectangle was called with the right colors
        # The main drawing calls are for the bounding boxes (thickness 2)
        # and for the text backgrounds (thickness -1)
        # We'll just verify the color arguments exist in the call history
        calls = mock_rectangle.call_args_list
        colors_used = [call[0][3] for call in calls]

        # BGR (255, 0, 0) is Blue = person
        self.assertIn((255, 0, 0), colors_used)
        # BGR (0, 0, 255) is Red = dog
        self.assertIn((0, 0, 255), colors_used)
        # BGR (0, 0, 0) is Black = car
        self.assertIn((0, 0, 0), colors_used)
...
class TestDetectSubjects(TestImageProcessor):
    ...
    def test_detect_subjects_no_detections(self, mock_load_image):
        """Test detect_subjects with no detections"""
        dummy_image = np.zeros((100, 100, 3), dtype=np.uint8)
        mock_load_image.return_value = dummy_image
...
class TestLoadImageHEIC(TestImageProcessor):
    ...
    def test_annotate_detections_uses_load_image_for_heic(self, mock_exif, mock_pil_open):
        ...
        # Mock the YOLO result to be empty
        mock_result = MagicMock()
        mock_result.boxes = None
        self.processor.yolo_model.return_value = [mock_result]
